  ...
  return (
    <div className={`flex-1 flex flex-col p-0 overflow-y-auto min-h-0`}>
      <div className="p-4 md:p-6">
        <h1 className="text-xl md:text-3xl font-semibold mb-2">Project Manager Dashboard</h1>
        <p className={`text-sm md:text-md ${darkMode ? 'text-slate-400' : 'text-slate-500'} mb-4 md:mb-6`}>
            Oversee your projects, manage tasks, and track team progress.
        </p>
        <Button onClick={openCreateProjectModal} variant="primary" size="md" className="mb-4 md:mb-6 w-full sm:w-auto justify-center">
            <ICON_MAP.PlusIcon className="w-5 h-5 mr-2" />
            Create New Project
        </Button>

        {managedProjects.length > 0 && (
          <div className={`mb-4 md:mb-6 p-4 rounded-xl ${darkMode ? 'bg-slate-800/50' : 'bg-slate-100/70'} border ${darkMode ? 'border-slate-700/50' : 'border-white/30'}`}>
            ...
            <div className="max-h-60 overflow-y-auto scrollbar-thin space-y-2 pr-2">
              {managedProjects.map(p => (
                <div key={p.id} className={`flex flex-col sm:flex-row justify-between items-start sm:items-center p-3 sm:p-2.5 rounded-md gap-2 sm:gap-0 ${darkMode ? 'bg-slate-700/50 hover:bg-slate-700' : 'bg-slate-50 hover:bg-slate-200'}`}>
                  <span className="truncate font-medium w-full sm:w-auto">{p.name}</span>
                  <Button size="sm" variant="outline" className="w-full sm:w-auto justify-center" onClick={() => handleViewProjectKanban(p.id)}>View Board</Button>
                </div>
              ))}
            </div>
          </div>
